...
This document is coded in R.  I've written a Python notebook [here]() with the code to do the same operations that you find in this document.
...
We are trying to understand a relationship between an outcome variable $y$ and a  an set of input variables $X = x_1, x_2, ..., x_p$ of the form 
...
In our last session we looked at a *continuous outcome* using a *linear model*:
...
Since a *transformation* of our outcome is linear on our input variable, we can create a model known as a *generalized linear model* which will model the log odds of our outcome event $y$.  As we will see, the coefficients of a model like this can be interpreted very intuitively to explain the impact of input variables on the likelihood of $y$ occurring.
...
```{r}
# run a binomial general linear model on the male individuals
model_m <- glm(dec ~ attr + intel + prob, data = speed_dating[speed_dating$gender == 1, ],
             family = "binomial")
...
Recalling the meaning of `(P > |z|)` - the p-value - we can determine that all three rating play a significant role in the decision outcome of a date where the individual is male.
...
Our coefficients indicate the linear impact on the log odds of a positive decision.  This means that a negative coefficient decreases the log odds and a positive coefficient increases the log odds.  In this context we can see that physical attractiveness and sense of reciprocation both have a positive effect on likelihood of a positive decision when the individual is male, but intelligence has a negative impact<sup>1</sup>.  
...
* $e^{\beta_0}$ is the odds of the event assuming zero from all input variables (the odds of someone perceived as very ugly, very dumb and very disengaged being viewed positively)
* $e^{\beta_i}$ is the multiplier of the odds associated with a one unit increase in $x_i$ (for example, an extra point rating in physical attractiveness), assuming all else equal - because of the mutiplicative effect, we call this the *odds ratio*


<font size=1>[1] I'd like to say I am surprised, but I'm not. </font>
---
...
We interpret our each of our odds ratios as follows (assuming all else equal):
...
* An extra point in perceived reciprocation of interest increases the odds by 32%
...
Increase in odds have a diminishing effect on probability as the original probability increases.  So it is important to know the difference between the two.  Here is a graph showing the impact of a 10% increase in odds on the probability of an event, depending on the original probability.
...
```{r}
# run a binomial general linear model on the females
model_f <- glm(dec ~ attr + intel + prob, data = speed_dating[speed_dating$gender == 0, ],
             family = "binomial")
...
Again, we see that all three ratings are significant, but we see different dynamics:

* An extra point in physical attractiveness increases odds by 74%
* An extra point in intelligence increases odds by 9%
* an extra point in perceived reciprocation of interest increases odds by 26%

We see attractiveness and intelligence both playing a positive role, and a lower effect of physical attractiveness when the decision maker is female.
...
Previously we looked at the fit of a linear regression model and determined a metric called $R^2$, which determined how much of the variance of $y$ was explained by our model.  This is not so straightforward in binomial logistic regression, and in fcat still the subject of research.  

Numerous variants of measures called *pseudo-$R^2$* exist to try to approximate something similar to an $R^2$.  The `DescTools` package provides easy access to these measures.  All have positive and negatives and should be handled carefully. Here are four of them.

```{r}
library(DescTools)
DescTools::PseudoR2(model_m, which = c("McFadden", "CoxSnell", "Nagelkerke", "Tjur"))
```
...
In linear regression we were able to compare our model to a completely random alternative and show with some level of certainty whether our model did a 'better than random' job.  This, again, is not straightforward with logistic regression.  Options includeL

* 'Goodness of fit' tests for inference involving breaking predictions into groups and comparing the observed prediction in each group with the modeled predictions in each group, and performing a chi-square test to determine if they are significantly different.  In these tests the null hypothesis is that there is a good fit, therefore (unlike linear regression) a low p-value is a sign of a poorly chosen model.  There are a number of different goodness of fit tests available in the `LogisticDx` package, including the Hosmer-Lemeshow test.
* Typical measures of predictive accuracy can be used if the model is being trained in a predictive context to classify test data, such as precision, recall and ROC-AUC.
...
Filter the data into separate sets according to `goal` - or combine sets if you think the goals are similar.  For each of your set, run a binomial logistic regression model and determine if there are any notable differences between the groups.  Feel free to share your findings!
...
## Next time: Multinomial Logistic Regression for outcomes that are categorical and without order
